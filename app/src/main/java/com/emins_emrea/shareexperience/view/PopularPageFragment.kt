...
class PopularPageFragment(private val filter: HashMap<String, String>) : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        popularRV.adapter = recyclerAdapter
        getPosts()
        val swipeRefresh: SwipeRefreshLayout = view.findViewById(R.id.swipeRefreshPopularPage)
    ...
    private fun getPosts() {
        ...
        if (filter.get("categoryFilter").isNullOrEmpty()) {
            db.collection("ExperiencePost").orderBy(
                "likesCount", Query.Direction.DESCENDING
            ...
            val postsRef = db.collection("ExperiencePost")
                .whereArrayContainsAny("categories", categoryList)
